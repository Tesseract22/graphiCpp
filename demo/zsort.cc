...
void render(int dt) {
    ...
    return [=](int x, int y) -> uint32_t {
      ...
      if (buf[y * W + x] > p.z) {
        ...
        if (vz < 0)
          vz = 0;
        vz = 100 - vz;
        return 0xff000000 | HSV2RGB((int)(360 + p.y) % 360, 100, vz);
        // return 0xff0000ff;
      }
      return 0;
    };
  ...
  for (int i = 0; i < flen; ++i) {
    ...
    cv.drawTriangle(cv_1.x, cv_1.y, cv_2.x, cv_2.y, cv_3.x, cv_3.y,
                    make_picker(cm_1, cm_2, cm_3, 120), false);
    // cv.drawTriangle(cv_1.x, cv_1.y, cv_2.x, cv_2.y, cv_3.x, cv_3.y,
    //                 CONST_PICKER(0xff0000ff), true);

    // cv.drawLine(100, 500, 900, 500, CONST_PICKER(0xffffff00));
    // cv.drawLine(cv_1.x, cv_1.y, cv_2.x, cv_2.y, CONST_PICKER(0Xff0000ff));
    // cv.drawLine(cv_3.x, cv_3.y, cv_2.x, cv_2.y, CONST_PICKER(0Xff0000ff));
    // cv.drawLine(cv_3.x, cv_3.y, cv_1.x, cv_1.y, CONST_PICKER(0Xff0000ff));
  }
